...
| `WIZARD_BYOM=1` | Ollama runtime + binary + loadout; the first run's "Run a model locally" reuses the install and pulls a tag sized to the hardware, `wizard --onboard` offers any tag (or set `WIZARD_MODEL=<tag>` to pull + write the config headlessly); see [byom.md](byom.md) |
...
The model is the first in the provider's table (`/model` lists the rest); mode is genie; there is no gateway, DuckDuckGo is the web search backend, and nothing is imported. The config is written to `~/.wizard/config.toml`, a sign-in prints its URL and waits for the browser, and Wizard opens the TUI with `saved ~/.wizard/config.toml · /setup changes it` on the card. Esc on a screen goes back one; Ctrl-C on any screen quits with nothing saved. Whenever the transcript is empty the card offers up to three starter prompts read off the directory (a README, uncommitted changes, a package manifest and its most-changed source file) as `❯` rows: ↓ then Enter runs one.

`contrib/first-run-pty.py --bin target/release/wizard --out /tmp/screens` times this on your machine (a fresh HOME under a pty): start to the first screen, Enter on the paste screen to the TUI prompt, and with `--scenario oauth` start to the xAI sign-in URL. The OAuth one is mostly the endpoint discovery request. Enter to the TUI prompt is the credential check: with no `WIZARD_PTY_KEY` the script runs under `unshare -rn`, so that figure is the offline case, and with a key it is one round trip to the provider (`--scenario compat` runs it against a local server the script starts). The check's verdict and how long it took are in the log (`WIZARD_LOG=info`). There are no checked-in numbers; run the script.
...
If Homebrew, pacman or Nix installed the binary (it resolves into a `Cellar`, to a `/usr/bin/wizard` that `pacman -Qo` owns, or into `/nix/store`), `wizard update` downloads nothing and prints that manager's command instead, and the startup notice names the same one: `brew upgrade wizard`, `sudo pacman -Syu wizard-bin`, `nix profile upgrade wizard`. Swapping a file behind a package database is how the next `brew upgrade` or `pacman -Syu` puts the old version back; `--force` does it anyway.
