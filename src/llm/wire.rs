...
//!
//! What does not live here is anything true of one endpoint only. The
//! `prompt_cache_key` field is the borderline case: two endpoints on this wire
//! document it (OpenAI's and xAI's) and the rest do not, so the key and the
//! endpoint test are here, in [`prompt_cache_key`] and
//! [`endpoint_takes_a_cache_key`], while deciding to install them stays with
//! each plugin.
...
/// API roots whose Chat Completions endpoint documents `prompt_cache_key`.
///
/// OpenAI's own, and xAI's: "A stable cache key for best-effort sticky routing
/// / prompt-cache hits across requests sharing a prompt prefix. Plumbed to
/// `x-grok-conv-id`" (docs.x.ai/developers/rest-api-reference/inference/
/// chat-completions, checked 2026-09-13). xAI's cache is per *server*, so
/// without the key a conversation's turns land on whichever machine answers
/// and a warm prefix is a coin flip; its own grok-4.6 page says so outright.
///
/// Nothing else in this family gets the field, and that is not a loss of
/// caching, only of routing affinity:
///
...
/// * DeepSeek and the other hosted endpoints reached as an `openai` provider
///   cache automatically and document no key field.
///
...
/// endpoint rather than on the wire shape.
const CACHE_KEY_API_ROOTS: &[&str] = &["https://api.openai.com", "https://api.x.ai"];

/// Whether `base_url` is one of [`CACHE_KEY_API_ROOTS`].
///
/// The comparison is anchored at the scheme and terminated at the path, so
/// `https://api.openai.com.example.net` and `https://api.x.ai.evil.test` are
/// not mistaken for the real ones. Callers pass the URL trimmed the way the
/// client trims it, so a configured trailing slash cannot decide whether the
/// key is sent.
pub(crate) fn endpoint_takes_a_cache_key(base_url: &str) -> bool {
    let base_url = base_url.trim_end_matches('/');
    CACHE_KEY_API_ROOTS.iter().any(|root| {
        match base_url.strip_prefix(root) {
            // What follows the host has to be a boundary: a path, a port, or
            // nothing.
...
pub(crate) fn prompt_cache_key(model: &str, messages: &[ChatMessage]) -> Option<String> {
...
mod tests {
    ...
    /// xAI's Chat Completions takes the same field, so the endpoint test has
    /// to say yes to it and no to a relay wearing its name.
    #[test]
    fn the_cache_key_endpoints_are_openai_and_xai_and_nothing_else() {
        for base_url in [
            ...
            assert!(endpoint_takes_a_cache_key(base_url), "{base_url}");
        }
        for base_url in [
    ...
    /// The shape an agent turn actually has: a system prompt, then tool calls
    /// and their results appended one step at a time. xAI's cache matches
    /// whole messages from the start of the array, so the key has to hold
    /// still across every step of a turn or the routing hint is worthless.
    #[test]
    fn the_key_holds_still_across_the_steps_of_one_turn() {
        let prompt = ChatMessage::system("You are Wizard.");
        let mut history = vec![prompt.clone(), ChatMessage::user("build it")];
        let first = prompt_cache_key("grok-4.6", &history).expect("a key");
        ...
        for step in 0..6 {
            ...
            history.push(ChatMessage::tool_result(&id, "execute", "a\nb\nc"));
            assert_eq!(
                prompt_cache_key("grok-4.6", &history).as_deref(),
                Some(first.as_str()),
                "appending a step must not re-key the conversation"
            );
        }
    }
}
