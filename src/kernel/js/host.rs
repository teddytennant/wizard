...
pub(crate) async fn apply(state: &VmState, script: &str, module_name: &str) -> anyhow::Result<()> {
...
pub(crate) fn call_function(
    ...
    async move {
        // The failure is flattened to a `String` *inside* the closure, while
        // the context is still in hand. `rquickjs::Error::Exception` prints as
        // "Exception generated by QuickJS" and carries nothing — the thrown
        // value is left in the context for the embedder to pick up — so an
        // error carried out and formatted afterwards has lost the plugin's
        // own message, which is the only part a plugin author can act on.
        let answer: Result<Value, String> = context
            .async_with(async |js| {
                let inner = async {
                    ...
                    let function: Function = table.get(func as usize).map_err(|_| {
                        rquickjs::Error::new_from_js_message(
                            "undefined",
                            "function",
                            "the plugin has no callback with that id",
                        )
                    })?;
                    ...
                    let answer = if answer.is_promise() {
                        answer
                            .into_promise()
                            .expect("checked")
                            .into_future::<JsValue>()
                ...
                inner.await.map_err(|err| thrown(&js, err))
            })
            .await;
        answer.map_err(|message| anyhow::anyhow!("{message}"))
    }
}
...
fn install_paths<'js>(js: &JsCtx<'js>, wizard: &Object<'js>, ctx: &Ctx) -> rquickjs::Result<()> {
    ...
    paths.set(
        "project",
        ctx.kernel().project_root().to_string_lossy().into_owned(),
    )?;
    for (key, path) in [
...
/// embedder to pick up. So every user-facing failure has to be flattened here,
/// while the context is still in hand, rather than carried out as an `Error`
/// and formatted later — which is a mistake that compiles, runs, and reports
/// that sentence to every plugin author forever.
...
fn emit_fn<'js>(js: &JsCtx<'js>, ctx: &Ctx) -> rquickjs::Result<Function<'js>> {
    ...
    Function::new(
        js.clone(),
        Async(
            move |cx: JsCtx<'js>, event: String, payload: Opt<JsValue<'js>>| {
                let ctx = ctx.clone();
