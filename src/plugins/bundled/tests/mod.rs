//! The bundled scripted plugins, exercised as tools.
//!
...
//! the way the dispatcher would. The fixtures say nothing about which language
//! a plugin is written in, which is the point — `git` and `publish` are Lua,
//! `json` is JavaScript, and every one of them is reached the same way.
//!
...
//! runs the plugin's own script, the `ctx` object, `wizard.process.exec`, the
//! shell runner and a real process, and reads the string that comes back out
//! the far end. A mock at any layer of that would test the mock. Where a state
//! cannot be *arranged* that way — a program that exits non-zero and says
//! nothing, one that outlives its budget, a `gh` nobody has installed — the
//! plugin's module supplies a `HostBridge` whose `exec` answers from a script,
//! which is one layer further out than the Rust tests used to sit and no
//! further.
