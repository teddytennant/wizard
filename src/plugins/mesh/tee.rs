...
//! node.
//!
//! [`super::Mesh::publish_turn`] has been built and tested since tier 2 landed
//! and nothing called it, which is the difference between a mesh that works and
//! a mesh that is reachable. This is the call site: one hook off
//! [`App::handle_agent_event`](crate::app::App::handle_agent_event), where the
//! same [`AgentEvent`] the TUI is about to render goes to whoever is watching.
//!
//! # How core reaches this without naming it
//!
//! It does not name it. [`MeshTee`] implements
//! [`SessionTee`](crate::app::SessionTee) — a core trait with three methods —
//! and [`MeshPlugin`](super::MeshPlugin) `provide`s a
//! [`TeeFactory`](crate::app::TeeFactory) under
//! [`SESSION_TEE`](crate::app::tee::SESSION_TEE) at kernel boot. `App::mesh` is
//! a `Box<dyn SessionTee>`, so a build without `--features mesh` has a `None`
//! there and no socket, and core did not change a line to get it. Every word
//! the user reads about this listening, or failing to, is written below rather
//! than in `src/app/runtime.rs`.
//!
...
//! [`super::quic`]). So with `listen = false` there is no way for anybody
...
//! [`PeerTurn::sanitize`](super::PeerTurn::sanitize). A second policy
...
use super::transport::PeerEventKind;
use super::{Capability, Identity, Mesh, NodeId, PeerStore, Transport};
use crate::agent::AgentEvent;
use crate::app::{SessionTee, TeeFactory};
...
/// The core-facing half: three methods over the inherent ones above.
///
/// Thin on purpose. The trait is core's vocabulary for "something takes this
/// session's events", and everything mesh-shaped — where it is bound, how many
/// peers are watching, this node's pasteable address — stays inherent, because
/// a trait that grew a `listening_at` would be core describing a socket.
impl SessionTee for MeshTee {
    fn publish(&self, event: &AgentEvent) -> usize {
        MeshTee::publish(self, event)
    }

    /// The plugin's own sentence, not core's.
    ///
    /// It names both the bound address and this node's pasteable one, because
    /// those two together are what somebody needs to check that the socket
    /// they did not ask for is the one their config asked for. `?` when the
    /// socket cannot say where it is: a notice that could not render an
    /// address is still a notice that the socket is open.
    ...
    fn leave(self: Box<Self>) -> Pin<Box<dyn Future<Output = ()> + Send>> {
        Box::pin(MeshTee::leave(*self))
    }
}

/// What [`MeshPlugin`](super::MeshPlugin) hands the kernel.
///
/// The failure is worded here rather than at the call site for the reason the
/// module docs give: `src/app/runtime.rs` prints `{err:#}` and nothing else, so
/// the whole sentence a user reads about a mesh that meant to listen and could
/// not has to be written by the thing that tried.
pub fn factory() -> TeeFactory {
    TeeFactory::new(|config, session| async move {
        match MeshTee::join(&config, &session).await {
            Ok(Some(tee)) => Ok(Some(Box::new(tee) as Box<dyn SessionTee>)),
            Ok(None) => Ok(None),
            Err(err) => Err(anyhow!(
...
mod tests {
    ...
    use crate::plugins::mesh::{PeerEvent, Subscription, Trust};
    ...
    fn the_console_variants_cross_as_reports_and_carry_no_writable_ticket() {
        ...
        for event in [
            ...
            let crossed = crate::plugins::mesh::PeerTurn::sanitize(&event)
