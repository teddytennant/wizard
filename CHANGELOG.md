...
- **Quality gates on sovereign runs (`--gate "<command>"`, repeatable).** A gate is a command that has to exit zero before the run is allowed to finish. Circuit breakers already caught a run that was failing; nothing caught a run that was merely wrong. Gates also come from `gates` in config and from the project's own `.wizard/gates.toml`. A failure feeds its bounded output back as another turn, and those remediation turns extend the loop bound rather than consuming it, or the default `--loop 1` would report a failure and never get a turn to fix it. A failed gate is not re-run while the workspace is unchanged, measured over tracked and untracked-but-not-ignored files, because a `cargo test` gate rewrites `target/` on every run and every recheck would otherwise look like fresh progress. Only failures are cached; a pass is always re-checked. A run that ends with a gate failing exits 5 whatever else stopped it. Genie mode never runs them: a gate stands in for a human reading the result, and in genie mode there is one. See [docs/modes.md](docs/modes.md).
- **Ctrl-Y copies the last reply.** A drag only reaches what is on screen, so a long answer could not be copied at all, and a serial console has no mouse path.
...
- **A failed exec-replace no longer leaves the process dying on a closed pipe.** `exec_replace` does not fork, and the standard library restores `SIGPIPE` to the default immediately before `execvp` so the new program does not inherit Rust's ignore. On success that is right and invisible; on failure the call returns and the ignore stays undone, so from then on `wizard ... | head` is killed by signal 13 instead of getting `EPIPE`. Deep evolve is the caller that matters, since the whole point of that error path is to leave a working agent behind when a swap fails. It showed up first as the test suite dying mid-run in a different place each time, depending on which test wrote to a closed pipe first.
- **Copying text out over SSH reaches the terminal you are actually sitting at.** Two causes. A bare OSC 52 does not survive tmux, and both doors are shut by default: `set-clipboard` is `external` and `allow-passthrough` has been off since 3.3a, so the DCS wrapper alone would not have fixed it either. `tmux load-buffer -w` is the route that works on stock settings, and it fills the paste buffer on the way. The larger cause is that the old code returned as soon as a native tool exited zero, so the escape was never sent at all: over SSH `xclip` writes to the remote clipboard, which nobody can see, and exits zero. Every applicable route now runs, and native tools go last when there is no local display. The escape is chunked for `screen`, and skipped past 74994 bytes rather than sent as a truncated sequence the terminal will silently drop, with the other routes still running and a notice saying what landed. Screen support is written to its documented limits but is untested.
...
- **Oversized tool output spills to a file instead of being dropped.** Truncation kept a head and a tail and told the model to rerun a narrower command, so recovering the middle meant paying for the command twice, and on something slow or with side effects the model usually guessed instead. The full result now goes to a private per-session file and the result keeps the same head and tail plus the path, so the model recovers the rest with `read_file` or `search_files`. Observed on a 2 MB command output: the model found the line it wanted by searching the spill path, five seconds after the call returned. `read_file` is exempt, since a read that spills and then tells the model to read the spill file is a loop. Spill files are owner-only inside an owner-only directory, created with `O_EXCL` so a planted symlink fails the write rather than redirecting it, and a failed spill falls back to plain truncation rather than failing the call.
- **Compaction prunes oversized tool results before paying for a summary.** The pass trims every over-budget tool result outside the recent window mechanically, and when that alone brings the context under the low-water mark it returns without calling the model at all. Pruning is idempotent and cuts on char boundaries. Only the in-memory history is touched; the session JSONL is append-only and still holds the full result.
...
[3.0.1]: https://github.com/teddytennant/wizard/compare/v3.0.0...v3.0.1
