...
- **Releases are signed, and the installer enforces it.** `install.sh` fetches `checksums.txt` and `checksums.txt.minisig`, verifies the signature under the key inlined in the script, then verifies each asset's SHA-256 against that file; `wizard update` applies the same rules with the key compiled in. Every failure aborts, and there is no flag to skip the check. Verifying needs one of three things on the host: `minisign`, an OpenSSL with ed25519 and blake2b, or `python3`. macOS has the third out of the box, which is what makes the one-liner work there: Apple's `openssl` is LibreSSL and can do neither half of the check. Homebrew and MacPorts prefixes are searched too, so a keg-only `openssl@3` counts. With none of the three, add `WIZARD_BUILD_FROM_SOURCE=1` to build from the release tag instead. See [Getting started](docs/getting-started.md#install).
